...
脚本只会把扩展客户端安装到本机 VS Code 扩展目录，不会复制运行器、
Language Server 或标准库。安装完成后，在 VS Code 中执行
**Developer: Reload Window**，然后打开 `.tap` 文件。

扩展按以下顺序寻找 Language Server：

1. `tapas.languageServer.path` 设置指定的路径；
2. 调试扩展时的源码仓库，或当前工作区中的 `build/bin/tapas-language-server`；
3. `PATH` 中的 `tapas-language-server`。
...
2. 调试扩展时的源码仓库，或当前工作区中的 `build/bin/tapas`；
3. `PATH` 中的 `tapas`。
...
- `extension.js` 把 Language Server 响应转换为 VS Code Provider，并管理文档和
  诊断的生命周期。
- `formatter.js` 负责隔离临时文件，并通过已安装的 Tapas Core 调用标准 `format` 包。
- `protocol.js` 实现 Content-Length 消息分帧，以及 JSON-RPC 请求、响应和通知。
- `language-configuration.json` 定义注释、括号、自动闭合和缩进。
- `syntaxes/tapas.tmLanguage.json` 定义无需服务器即可工作的 TextMate 后备高亮。
- `../../src/lsp/` 包含 C 实现的 Language Server。
- `../../src/compile/frontend/module.c` 提取与 VM 无关的模块公开接口，并提供默认环境清单。
- `../../src/compile/frontend/workspace.c` 管理文档覆盖、磁盘索引、导入图和跨模块解析。

扩展从 LSP `initialize` 响应读取语义 Token 图例，不复制 Token 类别。Language Server
直接复用文档已有的词法 Token、AST 和语义模型；标准名称在服务启动时从编译器和
标准库签名表建立只读索引。`tapas/syntaxCatalog` 仅供协议一致性测试读取，编辑期间
不会动态生成语法文件，也不会增加请求开销。
