...
```tapas
let arr = [2, 7, 3, 4, 3, 1, 9, 1, 8, 4, 6, 9, 5]
```
...
```tapas
let bubble_sort = (arr) {
	for (let i in std::toiter(arr.std::len(), -1, 0)) {
		for (let j in 0 to i - 1) {
			if (arr[j] > arr[j + 1]) {
				let mid = arr[j + 1]
				arr[j + 1] = arr[j]
				arr[j] = mid
...
```tapas
let arr_copy = arr.std::copy()   // first, get a copy of our list
...
let quick_sort: fn = (arr) {
	...
	let mididx = std::toint(arr.std::len() / 2 - 1)
