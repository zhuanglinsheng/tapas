...
本文定义 Tapas 下一阶段的类型系统，包括类型标注、运行时 `Type` 值、静态检查、
`types` 包和实现约束。文中尚未进入当前语言规范的语法和接口，均按拟议的正式
语义描述。
...
每条定义路径最终都必须到达 `types::AnyType`。本设计不支持直接或间接递归；
...
因此应先把构造结果绑定到名称；本设计不增加 `List[Int]` 一类语法：
...
后者检查容器中的所有键和值；本设计不能在同一个 Type 中组合这两种约束。
...
变量初始化、后续赋值和字段写入统一使用可赋值关系。以后增加函数参数和返回值
标注时，也应使用同一关系。
