...
uint_cmds size32()
{
	return static_cast<uint_cmds>(size());
}
...
uint_csts size32()
{
	return static_cast<uint_csts>(size());
}

/// Append a string to the end
void append(const std::string str)
{
	if (size32() == CST_LIMIT - 1)
...
uint_csts add_str_const(const std::string & str)
{
...
uint_csts add_int_const(const long & l)
{
...
uint_csts add_double_const(const double & d)
{
...
struct tcinfo
{
	uint_objs obj_max; ///< maximum number of environmental objects
	uint_objs tmp_max; ///< maximum number of temporary objects
	uint_regs reg_max; ///< maximum number of stack location used
	uint16_t padding_1;
};
...
{
	uint_csts ncints; ///< number of constant integers
	uint_csts ncstrs; ///< number of constant strins
	uint_csts ncdbls; ///< number of double floats
...
struct twrapper
{
	...
	uint_cmds ncmds;        ///< number of bycodes
};
...
void save_bin_file(const twrapper * wrapper, const std::string & file)
{
	...
	for (uint_csts i = 0; i < wrapper->consts.ncstrs; i++)
	{
		uint_lexs len_i = std::string(*str).size();
		fwrite(&len_i, sizeof(uint_lexs), 1, f);
...
twrapper * load_bin_file(const std::string & file)
{
	...
	uint_csts ncdbls = wrapper->consts.ncdbls;
...
void display_wrapper(const twrapper * wrapper)
{
	...
	for (uint_csts i = 0; i < wrapper->consts.ncints; i++)
	{
	...
	for (uint_csts i = 0; i < wrapper->consts.ncdbls; i++)
	{
