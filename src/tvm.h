...
void vmstk_pop_clean_front_n(uint_regs n)
{
...
/// OP_IDXR
void parse_idx(const uint_regs nparams)
{
	tobj & obj = vmstk_top();
	vmstk_pop_front();
...
template<typename T>
void parse_idx_basic(const uint_regs nparams, T * arr, tobj & obj)
{
...
{
	uint_regs nparams = static_cast<uint_regs>(iter->get_U());
	tobj & obj = vmstk_top();
...
/// OP_EVALTF (Not yet)
void parse_eval_tf(tcompo_v * v, tobj * params, uint_regs nparams, tcompo_env * env)
{
...
/// OP_IMPORT
void parse_import(const uint_csts cloc, char ** const clst, tcompo_env * const env)
{
	// load current lib
...
/// OP_IDXL
void parse_idxl(const uint_objs loc, const uint_regs nparams,
...
template<typename T>
void parse_idxl_basic(const uint_regs nparams, T * arr)
{
	tobj & rv = vmstk_at(nparams);
	tobj * params = vmstk_get_top_n(nparams);
...
template<typename T>
void parse_loopas_basic(T * p, uint_objs vloc, tobj & vre,
...
void parse_binop(const binopf & f, tbycode * iter,
			uint_regs type, tcompo_env * const env)
{
	...
	switch (type) {
	case 0: // value value
		f(vmstk_at(static_cast<uint_regs>(left)),
		  vmstk_at(static_cast<uint_regs>(right)),
		  vmstk_at(static_cast<uint_regs>(right)));
		vmstk_pop_clean_front();
		break;
	case 1: // env value
	...
	case 2: // value env
		f(vmstk_at(static_cast<uint_regs>(left)),
		  env->get_obj(right),
		  vmstk_at(static_cast<uint_regs>(left)));
		break;
	case 3: // env env
...
void exec_tin(tbycode *& iter, uint_cmds & idx, uint_cmds end, tcompo_env * env,
		long * cintlsts, double * cdbllsts, char ** cstrlsts)
{
	...
	case OP_RET: {
		if (vmstk_size() > 0) {
			...
			if (__rev.get_type() == tcompo && env->tenv_get_compo_type() == compo_tfunc) {
				...
				uint_objs loc = env->get_ref_obj_loc(__rev.get_v_tcompo());
	...
	case OP_POPN: {
		...
		for (uint_regs i = 0; i < static_cast<uint_regs>(iter->get_L()); i++) {
	...
	case OP_IMPORT: {
		parse_import(static_cast<uint_csts>(iter->get_U()), cstrlsts, env);
		break;
	}
	case OP_IDXR: {
		parse_idx(static_cast<uint_regs>(iter->get_U()));
		break;
	}
	case OP_EVAL: {
		parse_eval(iter, env);
	...
	case OP_IDXL: {
		uint_objs loc = iter->get_L();
		uint_regs nparams = iter->get_b();
		bool isenv = iter->get_i();
		parse_idxl(loc, nparams, isenv, env);
		break;
	}
	case OP_PUSHF: {
		uint_cmds ncmds = iter->get_U();
		uint_regs nparams = static_cast<uint_regs>(vmstk_top().get_v_tint());
		vmstk_pop_front();
		uint_regs fregmax = static_cast<uint_regs>(vmstk_top().get_v_tint());
		vmstk_pop_front();
		uint_objs ntmps = static_cast<uint_objs>(vmstk_top().get_v_tint());
		vmstk_pop_front();
		uint_objs nobjs = static_cast<uint_objs>(vmstk_top().get_v_tint());
		vmstk_pop_front();
	...
	case OP_ADD: {
		uint_regs type = static_cast<uint_regs>(vmstk_top().get_v_tint());
		vmstk_pop_front();
 ...
 */
tvm(uint_objs tmpmax) : tobj_array(tmpmax)
{
	__stklen = 0;
	__regmax = 0;
	__stk = nullptr;
...
void set_tmpmax(uint_objs tmpmax)
{
 ...
 */
void set_vmstack(tobj * vmstack, uint_regs nreg)
{
	__stk = vmstack;
	__regmax = nreg;
	__stklen = 0;
 ...
 */
void exec_tins(uint_cmds from, uint_cmds ncmds, tcompo_env * env)
{
 ...
 */
void eval_bycodes(uint_cmds from, tlib * lib)
{
...
/// print(v1, v2, ...)
inline void gen_print(tobj * const params, uint_regs len, tobj & vre)
{
...
/// sprt(v1, v2, ...)
inline void gen_sprt(tobj* const params, uint_regs len, tobj& vre)
{
	for (uint_regs i = 0; i < len; i++) {
		tobj* atom_i = params + i;
...
/// len(value)
inline void gen_len(tobj * const params, uint_regs len, tobj & vre)
{
	if (len != 1)
...
/// type(obj)
inline void gen_type(tobj * const params, uint_regs len, tobj & vre)
{
	if (len != 1)
...
/// copy(value)
inline void gen_copy(tobj * const params, uint_regs len, tobj & vre)
{
	if (len != 1)
...
/// identical(v1, v2)
inline void gen_identical(tobj * const params, uint_regs len, tobj & vre)
{
	if (len != 2)
...
/// tobool(value)
inline void to_bool(tobj * const params, uint_regs len, tobj & vre)
{
	if (len != 1)
...
/// toint(value)
inline void to_int(tobj * const params, uint_regs len, tobj & vre)
{
	if (len != 1)
...
/// todouble(value)
inline void to_double(tobj * const params, uint_regs len, tobj & vre)
{
	if (len != 1)
...
inline void to_pair(tobj * const params, uint_regs len, tobj& vre)
{
	if (len != 2)
...
inline void to_iter(tobj * const params, uint_regs len, tobj& vre)
{
	if (len != 3)
		twarn(ErrRuntime_ParamsCtr).warn("to_iter", "");
