...
void idx(const tobj * params, uint_regs nparams, tobj & idxre)
{
	if (nparams != 1)
...
void iset(const tobj * params, uint_regs nparams, const tobj & v)
{
	// Determine v
...
long len() const
{
	uint_lexs length_size_t = length();

	if (length_size_t > Tap_Long_MAX)
		twarn(ErrRuntime_IntOutOfRange).warn("tstr::len", "");
	return static_cast<long>(length_size_t);
}
...
tlist(tobj * const params, uint_regs len)
{
...
/// add cpp function
void add_cppf(const char * fname, cppf f, uint_regs nparams)
{
	tobj v_cppf;
...
inline void str_to_bool(tobj * const params, uint_regs len, tobj & vre)
{
	if (len != 1)
...
inline void str_to_int(tobj * const params, uint_regs len, tobj & vre)
{
	if (len != 1)
...
inline void str_to_double(tobj * const params, uint_regs len, tobj & vre)
{
	if (len != 1)
...
/// tostr(value)
inline void to_str(tobj * const params, uint_regs len, tobj & vre)
{
	if (len != 1)
...
/// tolist(v1, v2, ...)
inline void to_list(tobj * const params, uint_regs len, tobj & vre)
{
	vre.set_v(new tlist(params, len));
}

/// append(set, ele)
inline void set_append(tobj * const params, uint_regs len, tobj & vre)
{
	vre.set_nil();
...
/// append(set, ele, loc)
inline void set_insert(tobj * const params, uint_regs len, tobj & vre)
{
	vre.set_nil();
...
/// pop(set)
inline void set_pop(tobj * const params, uint_regs len, tobj & vre)
{
	vre.set_nil();
...
/// delete(set, loc)
inline void set_delete(tobj * const params, uint_regs len, tobj & vre)
{
	vre.set_nil();
...
/// union(set1, set2)
inline void set_union(tobj * const params, uint_regs len, tobj & vre)
{
	if (len != 2)
...
inline void dict_keys(tobj* const params, uint_regs len, tobj& vre)
{
	if (len != 1)
...
inline void dict_values(tobj* const params, uint_regs len, tobj& vre)
{
	if (len != 1)
...
/// now()
inline void time_now(tobj* const params, uint_regs len, tobj& vre)
{
