...
private:
	...
	uint_objs __loc_in_father_env;            /// Environment as a Tree

public:
tcompo_env_abstract(uint_objs objlst_cap, tcompo_env_abstract * father_env) : tobj_array(objlst_cap)
{
...
uint_objs get_loc_in_father_env() const
{
	return __loc_in_father_env;
}

/// @return the object located at `loc`
tobj & get_obj(uint_objs loc, tcompo_env_abstract * from = nullptr)
{
	if (from == nullptr) {
		...
		uint_objs from_loc = from->__loc_in_father_env;
...
void set_obj(uint_objs loc, const tobj & v, tcompo_env_abstract * from = nullptr)
{
	if (from == nullptr) {
		if (loc < get_objlst_len())
...
private:
	...
	uint_objs __tmpmax;              /// VM info
	uint_regs __regmax;              /// VM info
	uint_regs __nparams;             /// VM info
	uint_regs __dynamic_nparams = 0; /// VM info
	tcompo_type __compo_type;        /// Environment as a Tree

public:

tcompo_env(uint_objs objlst_cap, tcompo_env * father_env, uint_regs regmax,
		uint_regs tmpmax, uint_regs nparams, tcompo_type compo_type)
...
void set_nparams(uint_regs n)
{
	 __nparams = n;
}

/// Set the number of parameters inputted (for VM)
void set_dynamic_nparams(uint_regs n)
{
	__dynamic_nparams = n;
}

/// Set __regmax (for VM)
void set_regmax(uint_regs n)
{
	__regmax = n;
...
/// Set __tmpmax (for VM)
void set_tmpmax(uint_objs ntmps)
{
	__tmpmax = ntmps;
}
...
typedef void (* sessf)(tobj * const, uint_regs, tobj &, tcompo_env *);
...
public:
tfunc(uint_objs nlocals, tcompo_env * father_env, uint_regs reg_max,
		uint_objs tmpmax, uint_regs nparams,
		uint_cmds cmdloc, uint_cmds ncmds)
...
void assign_params(tobj * const params, uint_regs nparams)
{
	// settings: tcompo_env
	set_objlst_len(0);
...
uint_cmds get_cmdloc() const
{
	return __cmdloc;
}

/// @return number of commands
uint_cmds get_ncmds() const
{
	return __ncmds;
}
...
{
	uint_objs current_len = __default_v_names.size();
...
{
	uint_lexs loc = 0, len = paths.length();
...
tlist * listing_objects()
{
	...
	for (uint_objs idx = __default_v_names.size(); idx < get_objlst_len(); idx++) {
		tobj vi = get_obj(idx);
		std::string name;
