/**
 * @file tenv.h
 * @brief Declares core execution environments and library bindings.
 * @details Defines the environment state shared by bytecode execution, native
 * calls, and module bindings, including the narrow VM-service attachment.
 * @note This is an internal core interface, not the high-level embedding API;
 * applications should normally include `tapas/tsession.h` instead.
 */
#ifndef TAPAS_TENV_H
#define TAPAS_TENV_H
...
typedef struct tcompo_env {
	...
	const tvm_services *vm_services;
	void *vm_services_context;
} tcompo_env;
