...
int tcompo_runtime_type(tcompo_v *self, tobj *result)
{
	tobj_set_nil(result);
	if (!self || !self->vtable || !self->vtable->capabilities ||
	    !self->vtable->capabilities->runtime_type)
		return 0;
	self->vtable->capabilities->runtime_type(self, result);
	return result->type == tcompo && result->val.v_tcompo;
}

int tcompo_matches_type(tcompo_v *self, const tobj *type)
{
	if (!self || !self->vtable || !self->vtable->capabilities ||
	    !self->vtable->capabilities->matches_type)
		return -1;
	return self->vtable->capabilities->matches_type(self, type) != 0;
}
