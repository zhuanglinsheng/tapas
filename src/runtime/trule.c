...
tcompo_vtable trule_vtable = {
	...
	.tostring_abbr = trule_format_abbr, .tostring_full = trule_format_full
};
	...
	long index, unsigned depth)
{
    ...
    if (kind == 'T') {
        ...
        trule_term *capture = trule_term_new(trule_term_capture, ttypeval_builtin(tbuiltin_any), &payload, nullptr, 0);
...
static trule_item *trule_load_item(trule_ir *ir, const char **position, long *item_index)
{
	...
	trule_term *term = trule_term_new(trule_term_construct,
		kind != 'R' ? ttypeval_builtin(tbuiltin_bool) :
