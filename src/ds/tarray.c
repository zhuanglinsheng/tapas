...
typedef enum {
	TARR_ADD, TARR_SUB, TARR_MUL, TARR_DIV, TARR_POW,
		...
		for (size_t k = 0; k < left->cols; k++)
			for (size_t j = 0; j < right->cols; j++)
				out->data[i * out->cols + j] +=
					left->data[i * left->cols + k] *
					right->data[k * right->cols + j];
	return out;
}
	...
	if (other->type == tcompo && tobj_compo_type(other) == compo_tdarr) {
		...
		for (size_t i = 0; i < a->rows * a->cols; i++) {
			...
			r->data[i] = op == TARR_ADD ? x + y : op == TARR_SUB ? x - y :
				op == TARR_MUL ? x * y : op == TARR_DIV ? x / y : pow(x, y);
		}
		tobj_set_compo(out, (tcompo_v *)r);
		return;
	}
...
DEF_ARR_OP(div, TARR_DIV) DEF_ARR_OP(pow, TARR_POW) DEF_ARR_OP(eq, TARR_EQ)
...
tcompo_vtable tdarr_vtable = {
	...
	tdarr_add, tdarr_sub, tdarr_mul, tdarr_div, NULL, tdarr_pow, tdarr_mmul,
...
static const char *ttime_type(void) { return "Time"; }
static tcompo_type ttime_code(void) { return compo_time; }
static long ttime_len(void *self) { (void)self; return 0; }
static void *ttime_copy_impl(void *self) { return ttime_from_time(((ttime *)self)->value); }
static void ttime_free_impl(void *self) { free(self); }
static int ttime_identical_impl(void *self, void *other) { return ((ttime *)self)->value == ((ttime *)other)->value; }
static tstring *ttime_string(void *self)
{
	char buf[32];
	struct tm *tmv = localtime(&((ttime *)self)->value);
	if (!tmv || !strftime(buf, sizeof(buf), "%Y-%m-%d %H:%M:%S", tmv))
		return tstring_new("<invalid time>");
	return tstring_new(buf);
}
static void ttime_sub(void *self, const tobj *other, int rhs, tobj *out)
{
	if (other->type != tcompo || tobj_compo_type(other) != compo_time)
		twarn(ErrRuntime_ParamsType, "ttime_sub", "two time values required");
	double d = difftime(((ttime *)self)->value, ((ttime *)other->val.v_tcompo)->value);
	tobj_set_float(out, rhs ? -d : d);
}
...
ttime *ttime_from_time(time_t value)
{
	...
	if (!out) twarn(ErrRuntime_Other, "ttime_from_time", "out of memory");
	out->base.vtable = &ttime_vtable;
	out->value = value;
	return out;
}

ttime *ttime_new(void) { return ttime_from_time(time(NULL)); }
time_t ttime_get(const ttime *value) { return value->value; }
