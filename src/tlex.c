			 ...
			 tstring **right)
{
	...
	*right = tstring_substr(body, eq + 1, tstring_len(body) - eq - 1);
	...
	uint_lexs colon = 0;
	if (tlex_find_last_top_level_char_ts(sig, ':', &colon)) {
...
#define APPEND_TOKEN_MOVE(tok)                                                 \
	...
	/* kappa expression #{ ... } already handled above */

	/* Binary operators — check with char* operations */
	{
		/* in: the lowest-priority expression operator. */
		uint_lexs off = 0;
	...
	{
		/* idxl/asg: assignment has lower precedence than RHS expressions. */
		uint_lexs off_eq = 0;
	...
	}
	{
		...
		if (tlex_find_last_top_level_char_ts(unit_ts, '*', &off)) {
			if (off > 0 && unit[off - 1] == '*') {
				/* mmul */
				EMIT_BINARY_TS(token_mmul, off - 1, off + 1);
			} else {
				EMIT_BINARY_TS(token_mul, off, off + 1);
			}
		}
	}
	{
		uint_lexs off = 0;
		if (tlex_find_last_top_level_char_ts(unit_ts, '/', &off)) {
			EMIT_BINARY_TS(token_div, off, off + 1);
		}
	}
	{
		uint_lexs off = 0;
		if (tlex_find_last_top_level_char_ts(unit_ts, '%', &off)) {
			EMIT_BINARY_TS(token_mod, off, off + 1);
		}
	}
	{
		uint_lexs off = 0;
...
tstring *preprocessing_1_ts(const tstring *fullcmd)
{
	...
	if (tstring_len(cmd) > 0 && tstring_cstr(cmd)[0] == '-') {
		tstring *reform = tstring_new("0");
		tstring_append_ts(reform, cmd);
		tstring_free(cmd);
		cmd = reform;
	}
	return cmd;
