...
tstring *tworkspace_resolve_module_file(const char *path,
					 ...
					 uint32_t root_count)
{
	tstring *found = path && path[0] == '/' ? module_file(path) : nullptr;
...
{
	if (!workspace) return;
	tworkspace_reference_index_free(workspace);
...
{
	for (tworkspace_document *document = workspace ? workspace->documents : nullptr;
...
static void index_module_members(tworkspace *workspace,
				 tworkspace_document *document)
{
	tworkspace_reference_remove(workspace, document);
	for (tast_id id = 0; id < document->frontend.arena.node_count; id++) {
		...
		if (tworkspace_resolve_member_node(
		    workspace, document, id, &member)) {
			if (member.document && member.exported)
				tworkspace_reference_add(workspace, document,
					member.document, member.exported,
					member.reference_span);
			continue;
		}
...
static tstatic_type_id standard_type(void *context, tstatic_type_arena *arena,
	...
	int open)
{
	...
	tfrontend_init_with_resolver(&document->frontend,
		tstring_cstr(document->uri), source, tfrontend_module,
		standard_type, workspace);
...
tworkspace_document *tworkspace_update(tworkspace *workspace, const char *uri,
					const char *source, int64_t version)
{
	if (!workspace || !uri || !source) return nullptr;
...
{
	if (!workspace || !uri) return nullptr;
	tworkspace_document *document = tworkspace_find(workspace, uri);
...
{
	if (!document || !symbol || symbol->kind != tsemantic_symbol_import) return nullptr;
	for (uint32_t i = 0; i < document->import_count; i++)
				   ...
				   tworkspace_member_resolution *result)
{
	...
	const tast_node *node = document ? tast_get(&document->frontend.arena, member) : nullptr;
	if (!workspace || !node || node->kind != tast_member) return 0;
	if (node->member.op == tsyntax_dot) {
		tast_id parent = tcontrol_parent(&document->frontend.flow, member);
		const tast_node *call = tast_get(&document->frontend.arena, parent);
		if (!call || call->kind != tast_call ||
		    tcontrol_role(&document->frontend.flow, member) !=
			tcontrol_child_receiver) return 0;
		tstring *name = tsource_document_slice(
			&document->frontend.document, node->member.name);
		const tstandard_symbol *standard = tstandard_symbol_find(
			nullptr, tstring_cstr(name));
		tstring_free(name);
		if (!standard || standard->kind != tmodule_symbol_function) return 0;
		if (result) *result = (tworkspace_member_resolution){
			.standard = standard,
			.reference_span = node->member.name
		};
		return 1;
	}
	if (node->member.op != tsyntax_scope) return 0;
	tworkspace_namespace namespace = { 0 };
	if (!resolve_namespace(workspace, document, node->member.receiver,
				    ...
				    tworkspace_namespace *result)
{
	...
	tast_id expression = tcontrol_find_node_at(&document->frontend.flow,
	...
	const tstandard_symbol **result, tsource_span *reference_span)
{
	...
	if (!node || node->kind != tast_name ||
	    tsemantic_resolved_symbol(&document->frontend.semantic, id))
		return 0;
	tstring *name = tsource_document_slice(
		&document->frontend.document, node->span);
	const tstandard_symbol *found = nullptr;
	for (uint32_t i = 0; i < tstandard_symbol_count(); i++) {
		const tstandard_symbol *candidate = tstandard_symbol_at(i);
		if (!candidate->package &&
		    strcmp(candidate->name, tstring_cstr(name)) == 0) {
			found = candidate;
			break;
		}
	}
	...
	if (!found) return 0;
	if (result) *result = found;
	if (reference_span) *reference_span = node->span;
	return 1;
}
			      ...
			      tworkspace_member_resolution *result)
{
	...
	for (tast_id id = tcontrol_find_node_at(&document->frontend.flow,
		&document->frontend.arena, document->frontend.root, offset);
	     id != TAST_INVALID_ID;
	     id = tcontrol_parent(&document->frontend.flow, id)) {
		const tast_node *node = tast_get(&document->frontend.arena, id);
		if (node && node->kind == tast_member &&
		    node->member.name.start <= offset && offset <= node->member.name.end)
			return tworkspace_resolve_member_node(
				workspace, document, id, result);
	}
	return 0;
}
