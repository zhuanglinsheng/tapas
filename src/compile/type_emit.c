			  ...
			  int direct_constructor_callee)
{
	...
	case tast_call: {
		...
		ttypeval *signature = tast_infer_expression_type(
			emitter, node->aggregate.receiver, NULL);
		if (signature && signature->kind == ttype_kind_function) {
			uint_objs expected_count =
				ttypeval_function_parameter_count(signature);
			if (!ttypeval_function_variadic(signature) &&
			    node->aggregate.count != expected_count)
				twarn(ErrCompile_Other, "function call",
				      "argument count does not match function signature");
			uint_objs checked = node->aggregate.count < expected_count ?
				node->aggregate.count : expected_count;
			for (uint_objs i = 0; i < checked; i++) {
				ttypeval *expected =
					ttypeval_function_parameter_at(signature, i);
				if (expected && !tast_expression_assignable_to(
						emitter, arguments[i], expected))
					twarn(ErrCompile_Other, "function call",
	...
	case tast_declaration_statement:
		if (node->declaration_statement.has_initializer)
			validate_node(emitter,
				node->declaration_statement.initializer, 0);
		break;
...
{
	if (!function || function->kind != tast_function)
		return NULL;
				     ...
				     ttypeval **static_value)
{
	...
	tstatic_type_id shared = ttype_info_node_id(
		&emitter->frontend->types, id);
	if (shared != TSTATIC_TYPE_UNKNOWN) {
		ttypeval *resolved = compile_type_from_static(
				  ...
				  const ttypeval *target)
{
	if (!target)
		return 1;
	if (target->kind == ttype_kind_recursive && target->recursive_defined)
		return tast_expression_assignable_to(
			emitter, id, target->recursive_body);
	ttypeval *known = tast_infer_expression_type(emitter, id, NULL);
	if (known) {
		int assignable = compile_type_assignable(known, target);
		ttypeval_release(known);
