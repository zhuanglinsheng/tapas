			      ...
			      uint_lexs npaths)
{
	...
	const char **roots = npaths ?
		(const char **)calloc(npaths, sizeof(const char *)) : nullptr;
		...
		int inblk)
{
	...
	tstring *saved_source_ctx = source_ctx ? tstring_new(source_ctx) : nullptr;
	tstring *saved_file_ctx = file_ctx ? tstring_new(file_ctx) : nullptr;
	...
	tcp_init_preload(
		&sub_comp, default_names, cp->n_default_objs, nullptr,
		cp->preload_library, cp->interactive);
	...
	sub_comp.module_interface = nullptr;
	free(local_paths);
	...
	terror_set_file_context(
		saved_file_ctx ? tstring_cstr(saved_file_ctx) : nullptr,
		line_ctx,
		column_ctx);
	...
	if (cp->imports && cp->imports->count) {
		...
		cp->imports->files[cp->imports->count] = nullptr;
	}
