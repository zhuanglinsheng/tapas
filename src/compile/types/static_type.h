/**
 * @file static_type.h
 * @brief Declares the compiler's package-neutral static Type representation.
 * @details Defines static Type nodes and construction, parsing, template,
 * comparison, and formatting operations used by the frontend.
 * @note Static Types may carry nominal package identities, but never package
 * object layouts or implementation-specific dispatch.
 */
...
#define TSTATIC_TYPE_INVALID_APPLICATION (UINT32_MAX - 2)
...
typedef enum {
	...
	tstatic_type_parameter,
	tstatic_type_value_parameter,
	tstatic_type_exact_value,
	tstatic_type_template
} tstatic_type_kind;
...
typedef struct {
	...
	uint32_t template_type_parameter_count;
