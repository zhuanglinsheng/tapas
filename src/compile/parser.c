...
{
	tast_id *parameters = NULL;
	uint32_t count = 0;
	uint32_t capacity = 0;
	int variadic = consume(parser, tsyntax_ellipsis, NULL);
	if (!variadic) {
	...
	if (!consume(parser, tsyntax_rparen, NULL))
...
static tast_id parse_dictionary(tparser *parser, tsource_span opening)
{
	...
	while (peek(parser)->kind != tsyntax_rbrace &&
		...
		if (consume(parser, tsyntax_colon, NULL)) {
			if (mode == 2)
				tdiagnostics_add(parser->diagnostics, tdiagnostic_error,
					 peek(parser)->span,
...
static void append_statement(tast_id **statements, uint32_t *count,
			     uint32_t *capacity, tast_id statement)
{
	if (statement == TAST_INVALID_ID)
		return;
	if (*count >= *capacity) {
		*capacity = *capacity ? *capacity * 2 : 8;
		*statements = (tast_id *)realloc(
			*statements, *capacity * sizeof(tast_id));
		if (!*statements)
			abort();
	}
...
static tast_id parse_block(tparser *parser)
{
	const tsyntax_token *opening = NULL;
	if (!consume(parser, tsyntax_lbrace, &opening))
		return error_node(parser, peek(parser)->span, "block expected");

	tast_id *statements = NULL;
	uint32_t count = 0;
	uint32_t capacity = 0;
	uint32_t segment = parser->cursor;
	uint32_t at = parser->cursor;
	uint32_t parentheses = 0, brackets = 0, braces = 0;
	const tsyntax_token *closing = NULL;
	while (at < parser->tokens->count) {
		tsyntax_kind kind = parser->tokens->items[at].kind;
		int at_top = parentheses == 0 && brackets == 0 && braces == 0;
		if (kind == tsyntax_rbrace && at_top) {
			append_statement(&statements, &count, &capacity,
				parse_statement_range(parser, segment, at));
			closing = &parser->tokens->items[at];
			parser->cursor = at + 1;
			break;
		}
		if (at_top && (kind == tsyntax_newline ||
			       kind == tsyntax_semicolon)) {
			append_statement(&statements, &count, &capacity,
				parse_statement_range(parser, segment, at));
			segment = at + 1;
			at++;
		...
		switch (kind) {
		case tsyntax_lparen: parentheses++; break;
		case tsyntax_rparen: if (parentheses) parentheses--; break;
		case tsyntax_lbracket: brackets++; break;
		case tsyntax_rbracket: if (brackets) brackets--; break;
		case tsyntax_lbrace: braces++; break;
	...
	uint32_t children = tast_arena_add_children(
		parser->arena, statements, count);
	free(statements);
	return tast_arena_add(parser->arena, (tast_node){
		.kind = tast_block,
		.span = { opening->span.start,
			  closing ? closing->span.end : peek(parser)->span.start },
...
static tast_id parse_control_statement(tparser *parser,
				       const tsyntax_token *keyword,
				       tast_kind kind)
{
				   ...
				   const tsyntax_token *keyword)
{
	if (!consume(parser, tsyntax_lparen, NULL))
		return error_node(parser, peek(parser)->span, "'(' expected");
	int declares = consume(parser, tsyntax_kw_let, NULL);
	const tsyntax_token *name = peek(parser);
	if (name->kind != tsyntax_identifier)
		return error_node(parser, name->span, "loop binding expected");
	advance(parser);
	if (!consume(parser, tsyntax_kw_in, NULL))
...
tast_id tparser_parse_statement(tparser *parser)
{
	...
	tast_id statement;
	if (first->kind == tsyntax_kw_if || first->kind == tsyntax_kw_elif ||
	    first->kind == tsyntax_kw_while) {
		advance(parser);
		tast_kind kind = first->kind == tsyntax_kw_if ? tast_if_statement :
			first->kind == tsyntax_kw_elif ? tast_elif_statement :
			tast_while_statement;
		statement = parse_control_statement(parser, first, kind);
	} else if (first->kind == tsyntax_kw_else) {
		advance(parser);
		tast_id body = parse_block(parser);
		const tast_node *block = tast_get(parser->arena, body);
		statement = tast_arena_add(parser->arena, (tast_node){
			.kind = tast_else_statement,
			.span = { first->span.start, block->span.end },
			.control_statement = { TAST_INVALID_ID, body }
		});
		...
		if (peek(parser)->kind == tsyntax_comma) {
			tast_id *declarations = NULL;
			uint32_t count = 0, capacity = 0;
			append_statement(&declarations, &count, &capacity, statement);
			while (consume(parser, tsyntax_comma, NULL))
				append_statement(
