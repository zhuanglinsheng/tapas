...
tobj(uint_objs obj_loc = UNDEF_ENVLOC, uint_csts name_loc = UNDEF_NAMELOC)
{
	set_nil();
	__env_loc = obj_loc;
	__name_loc = name_loc;
...
tobj(bool bv, uint_objs obj_loc = UNDEF_ENVLOC, uint_csts name_loc = UNDEF_NAMELOC)
{
	set_type(tbool);
	set_v_tbool(bv);
	__env_loc = obj_loc;
...
tobj(long iv, uint_objs obj_loc = UNDEF_ENVLOC, uint_csts name_loc = UNDEF_NAMELOC)
{
	set_type(tint);
	set_v_tint(iv);
	__env_loc = obj_loc;
...
tobj(double dv, uint_objs obj_loc = UNDEF_ENVLOC, uint_csts name_loc = UNDEF_NAMELOC)
{
	set_type(tdouble);
	set_v_tdouble(dv);
	__env_loc = obj_loc;
...
tobj(tcompo_v * cv, uint_objs obj_loc = UNDEF_ENVLOC, uint_csts name_loc = UNDEF_NAMELOC)
{
	set_type(tcompo);
	set_v_tcompo(cv);
	__env_loc = obj_loc;
...
void set_name_loc(uint_csts loc)
{
	__name_loc = loc;
}

/// Set the location of object in list of environmental objects.
void set_env_loc(uint_objs loc)
{
	__env_loc = loc;
}
...
uint_objs get_obj_loc() const
{
	return __env_loc;
}

/// Get the location of name in constant list of strings
uint_csts get_name_loc() const
{
	return __name_loc;
}
...
private:
	tobj * __objlst;
	uint_objs __objlst_cap;
	uint_objs __objlst_len;

public:
tobj_array(uint_objs objlst_cap)
{
...
{
	// Clean object list
	for (uint_objs i = 0; i<__objlst_cap; i++) {
...
void try_expand_objlist(uint_objs obj_max)
{
...
uint_objs get_objlst_cap()
{
	return __objlst_cap;
}

/// @return the length of current object array
uint_objs get_objlst_len() const
{
	return __objlst_len;
}

/// @return the location of the composition object `pv`
uint_objs get_ref_obj_loc(tcompo_v * pv) const
{
...
/// Decleare an object
void add_obj(uint_csts nameloc = UNDEF_NAMELOC)
{
...
void set_objlst_len(uint_objs nleft)
{
	__objlst_len = nleft;
}

bool has_obj(uint_csts nameloc)
{
	for (uint16_t i = 0; i<__objlst_len; i++) {
		tobj vi = __objlst[i];
...
tobj & get_obj(uint_objs n)
{
	return __objlst[n];
}

/// Set the object at `loc` to be `v`
void set_obj(uint_objs loc, const tobj & v)
{
...
void iset(const tobj * params, uint_regs np, const tobj & v)
{
	if (np != 1)
		twarn(ErrRuntime_ParamsCtr).warn("tpair::idx", "1 parameter");
	if (params->get_type() != tint)
...
virtual ~tcompo_idx() {};
/// arr[params]
virtual void idx(const tobj * params, uint_regs np, tobj& vre) = 0;
/// arr[params] = v
virtual void iset(const tobj * params, uint_regs np, const tobj& v) = 0;
};
...
/// C++ General Functions
typedef void (* cppf)(tobj * const, uint_regs, tobj &);
...
tcppgenf(const cppf & f, const char * name, uint_regs nparams)
{
