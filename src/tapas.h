...
inline void lib_ls(tobj* const params, uint_regs len, tobj& vre, tcompo_env * env)
{
	if (len != 1 && len != 0)
...
inline void lib_path(tobj* const params, uint_regs len, tobj& vre, tcompo_env * env)
{
	if (len != 0 && len != 1)
...
inline void tf_param(tobj* const params, uint_regs len, tobj& vre, tcompo_env * env)
{
	if (env->get_father_env() == nullptr)
...
inline void tf_nparam(tobj * const params, uint_regs len, tobj& vre, tcompo_env * env)
{
	if (env->get_father_env() == nullptr)
 ...
 */
tobj & get_obj(uint_objs loc)
{
	uint_objs len = __lib->get_objlst_len();
